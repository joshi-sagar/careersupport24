...
$result= mysql_query ("select * from user where status= '1' order by reg_date desc");

echo "<br>";
echo "<table border=5 >";

echo "<tr><th>Reg._Date</th><th>Name</th><th>Phone No</th> <th>User_type</th><th>ID_Document</th><th colspan=2 width=10%>Action</th></tr>";

while($row=mysql_fetch_array($result))
{

   echo "<tr><td width=5%> $row[13] </td><td width=10%> $row[5] </td><td width=20%> $row[14] </td><td width=20%> $row[17] </td><td width=10%> <a href='$row[23]'>Click </a></td>" . 

     "<td width=4% align = 'left' > <a href='pending2.php?id2=". $row[1] ."'><img src='images/Delete.png'  alt='delete'></a>" .

     "<a href = 'member_profile_for_job.php?id2=". $row[0] ." '><img src='images/Edit.png'></a></td></tr>" ;
