...
?> 
        ...
        <h1>All Deleted Applications</h1>
        &nbsp;&nbsp;<div id="comments">
          <?php


$result= mysql_query ("select * from user where status= '2' order by reg_date desc");
...
while($row=mysql_fetch_array($result))
{
   ...
   echo "<tr><td width=5%> $row[13] </td><td width=10%> $row[5] </td><td width=20%> $row[14] </td><td width=20%> $row[17] </td><td width=10%> <a href='$row[23]'>Click </a></td><td><a href = 'member_profile_for_job.php?id2=". $row[0] ." '><img src='images/Edit.png'></a></td></tr>" ;
